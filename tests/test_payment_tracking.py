...
def test_expense_model_has_payment_fields(client):
    """Тест что модель расходов содержит поля оплаты"""
...
def test_payment_status_filter_exists():
    ...
    # Фильтр "оплачено"
    paid = [e for e in expenses if e.get("is_paid") == True]
    assert len(paid) == 1
    assert paid[0]["id"] == 1
    
    # Фильтр "не оплачено"
    unpaid = [e for e in expenses if e.get("is_paid") == False or e.get("is_paid") is None]
    assert len(unpaid) == 2
...
def test_payment_status_display():
    ...
    paid_expense = {"is_paid": True}
    unpaid_expense = {"is_paid": False}
    
    # Проверяем правильные лейблы
    paid_label = "Оплачено" if paid_expense["is_paid"] else "К оплате"
    unpaid_label = "Оплачено" if unpaid_expense["is_paid"] else "К оплате"
    
    assert paid_label == "Оплачено"
    assert unpaid_label == "К оплате"
    
    # Проверяем цвета
    paid_color = "success" if paid_expense["is_paid"] else "warning"
    unpaid_color = "success" if unpaid_expense["is_paid"] else "warning"
