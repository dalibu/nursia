...
from database.models import ExpenseCategory, User, UserRole
...
async def test_create_category_unauthorized(client):
    ...
    response = client.post("/api/expenses/categories", json={"name": "Test"})
    assert response.status_code == 401
...
async def test_update_category_unauthorized(client):
    ...
    response = client.put("/api/expenses/categories/1", json={"name": "Test"})
    assert response.status_code == 401
...
async def test_delete_category_unauthorized(client):
    ...
    response = client.delete("/api/expenses/categories/1")
    assert response.status_code == 401
